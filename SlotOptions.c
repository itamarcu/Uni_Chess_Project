...
widget_t *create_slot_options(
    ...
    slot_options_t *data = (slot_options_t *) malloc(sizeof(slot_options_t));
    if (data == NULL) {
        free(res);
        return NULL;
    }
    ...
    SDL_Surface *blank_slot_surface = SDL_LoadBMP(BLANK_SLOT_PATH);
    ...
    SDL_Surface *numbered_slot_surface = SDL_CreateRGBSurface(0, SLOT_WIDTH, SLOT_HEIGHT, 32, 0, 0, 0, 0);
    ...
    for (int slot_index = 0; slot_index < number_of_slots; slot_index++) {
        SDL_BlitSurface(blank_slot_surface, NULL, numbered_slot_surface, &slot_rect_dst);
    ...
    SDL_FreeSurface(numbered_slot_surface);
    SDL_FreeSurface(blank_slot_surface);
    SDL_Surface *arrow_up_surface = SDL_LoadBMP(ARROW_UP_PATH);
    ...
    SDL_Texture *arrow_up_texture = SDL_CreateTextureFromSurface(window->renderer, arrow_up_surface);
    ...
    SDL_Surface *arrow_down_surface = SDL_LoadBMP(ARROW_DOWN_PATH);
    ...
    SDL_Texture *arrow_down_texture = SDL_CreateTextureFromSurface(window->renderer, arrow_down_surface);
